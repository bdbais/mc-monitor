...
APK pronto all'uso: **`MC-Monitor-1.25.apk`** (firmato, `minSdk 26` / Android 8+, `targetSdk 35`).
...
- **Macro** — file di comandi da lanciare in un tocco, con una ventina già pronte (manutenzione con conto alla rovescia, backup a caldo, pulizia degli oggetti a terra) e la possibilità di scriverne altre. `<giocatore>` diventa una domanda, `!attendi 30` è una pausa. Volendo se la fa scrivere da un servizio AI a scelta (Google AI Studio o Groq) con una chiave gratuita dell'utente: nell'app non ce n'è nessuna, e quello che torna indietro passa da una lista di comandi ammessi prima di diventare una macro. Vedi [Le macro](MANUALE.md#11-le-macro).
...
- **Mod** — cosa è installato sul server, ricerca su Modrinth con installazione lato server e verifica sha1, elenco dei mod più usati con il perché di ognuno, importazione di modpack `.mrpack`, installazione di Fabric su un server vanilla. Vedi [Mod e modpack](MANUALE.md#14-mod-e-modpack).
