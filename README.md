...
- **Macro** — file di comandi da lanciare in un tocco, con una ventina già pronte (manutenzione con conto alla rovescia, backup a caldo, pulizia degli oggetti a terra) e la possibilità di scriverne altre. `<giocatore>` diventa una domanda, `!attendi 30` è una pausa. Volendo se la fa scrivere da un servizio AI a scelta (Google AI Studio o Groq) con una chiave gratuita dell'utente: nell'app non ce n'è nessuna, e quello che torna indietro passa da una lista di comandi ammessi prima di diventare una macro. Vedi [Le macro](MANUALE.md#12-le-macro).
...
- **Ammettere e bannare su più server** — chi ha due mondi sulla stessa macchina non ha due comunità. Ammetti, banna, sbanna e togli dalla whitelist chiedono su quali altri server ripeterlo (quelli sullo stesso computer arrivano già spuntati), dicono dove sono andati e dove no con il motivo, e sanno disfarsi esattamente dove erano riusciti. Vedi [Ammettere e bannare su più server](MANUALE.md#ammettere-e-bannare-su-più-server).
- **L'inventario di un giocatore** — cosa ha addosso e nello zaino, disposto come nel gioco. Si legge dal file `playerdata` del server, con un lettore NBT scritto apposta; per chi è collegato l'app chiede prima al server di salvare, e dice sempre a quando si riferisce quello che vedi. Nessuna richiesta a Mojang. Vedi [L'inventario di un giocatore](MANUALE.md#linventario-di-un-giocatore).
- **Portare le liste su un altro server** — se il secondo mondo è arrivato dopo, la sua whitelist è vuota. L'app confronta le due liste, dice chi manca di là con i nomi, e porta solo quelli: non toglie mai nessuno, e non tocca chi è ammesso di qua e bannato di là. Vedi [Portare le liste su un altro server](MANUALE.md#portare-le-liste-su-un-altro-server).
...
- **Mod** — cosa è installato sul server, ricerca su Modrinth con installazione lato server e verifica sha1, elenco dei mod più usati con il perché di ognuno, importazione di modpack `.mrpack`, installazione di Fabric su un server vanilla. Vedi [Mod e modpack](MANUALE.md#15-mod-e-modpack).
