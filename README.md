...
APK pronto all'uso: **`MC-Monitor-1.23.apk`** (firmato, `minSdk 26` / Android 8+, `targetSdk 35`).
...
- **Macro** — file di comandi da lanciare in un tocco, con una ventina già pronte (manutenzione con conto alla rovescia, backup a caldo, pulizia degli oggetti a terra) e la possibilità di scriverne altre. `<giocatore>` diventa una domanda, `!attendi 30` è una pausa. Volendo se la fa scrivere da un servizio AI a scelta (Google AI Studio o Groq) con una chiave gratuita dell'utente: nell'app non ce n'è nessuna, e quello che torna indietro passa da una lista di comandi ammessi prima di diventare una macro. Vedi [Le macro](MANUALE.md#9-le-macro).
...
- **Impostazioni tecniche** — il file di configurazione di LinuxGSM (memoria, backup, avvisi), con la spiegazione di ogni chiave e quando avrà effetto. Vedi [Le impostazioni tecniche](MANUALE.md#6bis-le-impostazioni-tecniche-di-linuxgsm).
